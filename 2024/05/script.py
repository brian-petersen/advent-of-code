...
def parse_input(file):
    raw_rules, raw_updates = file.split('\n\n')

    rules = []
    for r in raw_rules.strip().split('\n'):
        a, b = r.split('|')
        rules.append((int(a), int(b)))

    updates = []
    for u in raw_updates.strip().split('\n'):
        updates.append([int(a) for a in u.split(',')])
...
def is_update_valid(update, rules_map):
    reversed = update[::-1]

    for i, e in enumerate(reversed):
        not_allowed = rules_map.get(e, set())
        predecessors = reversed[i+1:]
        if len(not_allowed.intersection(predecessors)) != 0:
            return False
...
#     filtered_updates = [u for u in updates if is_update_valid(u, rules_map)]
#
#     middles = [u[len(u) // 2] for u in filtered_updates]
#     print(sum(middles))
...
#
#     - 75,97,47,61,53 becomes 97,75,47,61,53. - 61,13,29 becomes 61,29,13. -
...
def fix_update(update, rules_map):
    attempts = 100
    while not is_update_valid(update, rules_map):
        attempts -= 1
        if attempts < 0:
            raise RuntimeError("Could not fix update")

        update = do_fix_update(update, rules_map)

    return update


# Makes one swap of updates if a rule violation is found
def do_fix_update(update, rules_map):
    update = list(reversed(update))

    for i, e in enumerate(update):
        not_allowed = rules_map.get(e, set())
        predecessors = update[i+1:]

        latest_violation = None
        for bad_e in not_allowed.intersection(predecessors):
            index = update.index(bad_e)
            if latest_violation is None or index > latest_violation:
                latest_violation = index

        if latest_violation:
            update[i], update[latest_violation] = update[latest_violation], update[i]
            return list(reversed(update))
    return list(reversed(update))
...
if __name__ == '__main__':
    ...
    rules, updates = parse_input(file)
    rules_map = build_rules_map(rules)
    bad_updates = [u for u in updates if not is_update_valid(u, rules_map)]
    fixed_updates = [fix_update(u, rules_map) for u in bad_updates]

    middles = [u[len(u) // 2] for u in fixed_updates]
    print(sum(middles))
