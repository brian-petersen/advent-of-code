...
def is_problem_solvable(problem, ops):
    result, operands = problem

    for operators in itertools.product(ops.keys(), repeat=len(operands) - 1):
        current = operands[0]
...
#     }
#
#     problems = [parse_problem(l) for l in lines]
#     solvable = [p for p in problems if is_problem_solvable(p, ops)]
#     print(sum([r for r, _ in solvable]))
